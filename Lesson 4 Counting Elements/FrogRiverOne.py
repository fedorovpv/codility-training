...
# moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the current in
...
def solution(X, A):
    leaves = [False] * X
    n = X
    for time, index in enumerate(A):
        if not leaves[index - 1]:
            leaves[index - 1] = True
            n -= 1
        if n == 0:
            return time
